...
export default function LetterAdventureGame() {
  ...
  useEffect(() => {
    ...
    const drawKopisto = (ctx: CanvasRenderingContext2D, x: number, y: number, dir: number, frame: number, isMoving: boolean) => {
      ctx.save()
      ...
      ctx.fillStyle = '#8B5CF6'
      ctx.beginPath()
      ctx.moveTo(8, 4); ctx.lineTo(2, -8); ctx.lineTo(14, 0); ctx.fill()
      ctx.beginPath()
      ctx.moveTo(26, 2); ctx.lineTo(34, -8); ctx.lineTo(30, 6); ctx.fill()

      // Inner ears
      ctx.fillStyle = '#DDD6FE'
      ctx.beginPath()
      ctx.moveTo(9, 2); ctx.lineTo(5, -4); ctx.lineTo(13, 1); ctx.fill()
      ...
      ctx.beginPath(); ctx.arc(15, 11, 1.2, 0, Math.PI * 2); ctx.fill()
      ctx.beginPath(); ctx.arc(25, 11, 1.2, 0, Math.PI * 2); ctx.fill()
      ...
      // Legs
      const legOff = isMoving ? Math.sin(frame * 0.3) * 5 : 0
      ctx.fillStyle = '#7C3AED'
      ctx.beginPath(); ctx.roundRect(8, 44, 8, 8 + legOff, 3); ctx.fill()
      ctx.beginPath(); ctx.roundRect(24, 44, 8, 8 - legOff, 3); ctx.fill()

      // Shoes
      ctx.fillStyle = '#5B21B6'
      ctx.beginPath(); ctx.roundRect(6, 50 + legOff, 12, 5, 2); ctx.fill()
      ctx.beginPath(); ctx.roundRect(22, 50 - legOff, 12, 5, 2); ctx.fill()
    ...
    const gameLoop = () => {
      ...
      if (g.gameStatus !== 'playing') {
        ...
        drawKopisto(ctx, g.playerX - g.cameraX, g.playerY, g.direction, g.frameCount, false)
        drawHUD(ctx)
      ...
      for (const p of ld.platforms) drawPlatform(ctx, p, g.cameraX)
      for (const orb of ld.letters) drawLetterOrb(ctx, orb, g.cameraX, g.frameCount, orb.order === g.nextLetterIdx)
      drawKopisto(ctx, g.playerX - g.cameraX, g.playerY, g.direction, g.frameCount, isMoving)
      drawHUD(ctx)
